...
position = list(itertools.product(
    ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'O', 'P'], range(1, 25)))
pos = []
...
num_fit_row = trunc(24/len(doses))

# Now, we have to calculates how many wells are used in a row.
num_used_wells = num_fit_row*len(doses)
...
row_length = len(doses)*num_fit_row
...
for i in comp:
    for i in range(numRepeats):
        ...
        c += 1
        if c == num_fit_row*numRepeats*16:
            c = 0
...
# Now, let's combine our dataframes.
    ...
    df = {'Plate Number': platenum_list,
          'Compound': compound_list, 'MW': mw_list}
    out_df = pd.DataFrame.from_dict(df, orient="index")
    out_df = out_df.transpose()
    out_df.columns = ['Plate Number', 'Compound', 'MW']
    out_df = pd.concat([out_df, df_pos, df_conc], axis=1,
